...
create type public.wallet_method as enum ('mpesa', 'mola', 'mcash', 'bank');
create type public.tx_kind as enum ('deposit','withdrawal','bet','prize','refund','fee','adjustment','bonus');
create type public.tx_status as enum ('pending','completed','failed','reversed');
create type public.room_status as enum ('WAITING','READY','STARTING','PLAYING','FINISHED','CANCELLED');
...
create index match_results_user_idx on public.match_results (user_id, created_at desc);
grant select on public.match_results to authenticated;
grant all on public.match_results to service_role;
alter table public.match_results enable row level security;
create policy "own results" on public.match_results for select to authenticated using (auth.uid() = user_id);
create policy "results public for ranking" on public.match_results for select to authenticated using (true);
...
grant execute on function public.credit_wallet(uuid,bigint,public.tx_kind,text,text,public.wallet_method,text,jsonb) to service_role;
grant execute on function public.debit_wallet(uuid,bigint,public.tx_kind,text,text,public.wallet_method,text,jsonb) to service_role;

-- one-time admin bootstrap guard
...
insert into public.admin_bootstrap (id) values (1);
